...
const REPORTS_DATA = {
  sales: [
    ...
    },
    {
      ...
      repName: "Ahmed Mostafa",
      area: "Nasr City",
      product: "Vitamin D Drops 1000IU",
      target: 12000,
      actual: 13000,
      repId: "rep1",
      dmId: "dm1",
...
function syncReportsData() {
  if (typeof window.DEMO_DATA !== "undefined" && window.DEMO_DATA) {
    ...
    if (
      Array.isArray(window.DEMO_DATA.doctors) &&
      window.DEMO_DATA.doctors.length > 0
    ) {
      window.DEMO_DATA.doctors.forEach((doc) => {
        if (
          !REPORTS_DATA.doctors.some(
            (d) => d.id === doc.id || d.name === doc.name,
          )
        ) {
    ...
    if (
      Array.isArray(window.DEMO_DATA.hospitals) &&
      window.DEMO_DATA.hospitals.length > 0
    ) {
      window.DEMO_DATA.hospitals.forEach((hosp) => {
        if (
          !REPORTS_DATA.doctors.some(
            (d) => d.id === hosp.id || d.name === hosp.name,
          )
        ) {
          REPORTS_DATA.doctors.push({
            id: hosp.id,
    ...
    if (
      Array.isArray(window.DEMO_DATA.visits) &&
      window.DEMO_DATA.visits.length > 0
    ) {
      window.DEMO_DATA.visits.forEach((v) => {
        const existingIdx = REPORTS_DATA.visits.findIndex(
          (rv) => rv.id === v.id,
        );
        const rep = (window.DEMO_DATA.users || []).find(
          (u) => u.id === v.repId,
        );
        ...
        const doc = REPORTS_DATA.doctors.find(
        ...
        const mappedVisit = {
          ...
          specialty: doc
            ...
            : v.period && v.period.toLowerCase() === "am"
              ...
              : "General",
          type:
            v.period && v.period.toLowerCase() === "am" ? "hospital" : "doctor",
          date: v.date,
          time: v.time || "10:00",
...
const directoryTranslations = {
  en: {
    ...
    doctorsDirectorySubtitle:
      "Master physician registry with contact details, specialty, and assignment.",
    pharmaciesDirectoryTitle: "💊 Pharmacies Directory",
    pharmaciesDirectorySubtitle:
      "Directory of authorized partner pharmacies, contact personnel, and assigned territories.",
    ...
    addDoctorSubtitle:
      "Register a new healthcare professional and assign territory.",
    ...
    addPharmacySubtitle:
      "Register an authorized partner pharmacy, contact pharmacist, and assigned representative.",
    editPharmacySubtitle:
      "Update pharmacy details, contact person, and assignment.",
    ...
    importDoctorsSubtitle:
      "Download our standardized English template, fill physician data, and upload for instant bulk import.",
    importPharmaciesTitle: "Import Pharmacies from Sheet",
    importPharmaciesSubtitle:
      "Download our standardized English template, fill pharmacy data, and upload for instant bulk import.",
    step1DownloadTemplate: "Step 1: Download Official Template (English Only)",
    step1Desc:
      "Use this official pre-formatted CSV template. Mandatory columns: Doctor Name, Specialty, Class, Address.",
    step1PharmDesc:
      "Use this official pre-formatted CSV template. Mandatory columns: Pharmacy Name, Address.",
    btnDownloadTemplate: "Download Template (.csv)",
    step2AssignRep: "Step 2: Assign Medical Representative",
    step2Desc:
      'Assign all imported records to a specific representative, or use the "Rep Code" specified in the sheet.',
  ...
  ar: {
    ...
    doctorsDirectorySubtitle:
      "سجل الأطباء المعتمد مع تفاصيل الاتصال، التخصص، والمنطقة التابعة.",
    pharmaciesDirectoryTitle: "💊 دليل الصيدليات",
    pharmaciesDirectorySubtitle:
      "سجل الصيدليات الشريكة، بيانات الصيدلي المسؤول، والمنطقة التابعة.",
    ...
    addDoctorSubtitle:
      "تسجيل طبيب جديد في الدليل وتعيين المندوب والمنطقة التابعة.",
    ...
    editPharmacySubtitle:
      "تعديل بيانات الصيدلية، الصيدلي المسؤول، والمنطقة التابعة.",
    deletePharmacy: "حذف صيدلية",
    confirmDeletePharmacy:
      "هل أنت متأكد من حذف هذه الصيدلية من النظام نهائياً؟",
    ...
    importDoctorsSubtitle:
      "قم بتحميل القالب الرسمي المعتمد باللغة الإنجليزية، املأ البيانات، وارفع الملف للاستيراد الفوري.",
    importPharmaciesTitle: "استيراد صيدليات من شيت إكسيل",
    importPharmaciesSubtitle:
      "قم بتحميل القالب الرسمي المعتمد باللغة الإنجليزية، املأ البيانات، وارفع الملف للاستيراد الفوري.",
    step1DownloadTemplate: "الخطوة 1: تحميل القالب الرسمي (باللغة الإنجليزية)",
    step1Desc:
      "استخدم هذا القالب الرسمي. الحقول الإلزامية: Doctor Name, Specialty, Class, Address.",
    step1PharmDesc:
      "استخدم هذا القالب الرسمي. الحقول الإلزامية: Pharmacy Name, Address.",
    btnDownloadTemplate: "تحميل القالب (.csv)",
    step2AssignRep: "الخطوة 2: تعيين المندوب المسؤول",
    step2Desc:
      "عيّن جميع السجلات المرفوعة لمندوب محدد، أو اتركها لتقرأ من عمود Rep Code في الشيت.",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (window.translations) {
    window.translations.en = {
      ...window.translations.en,
      ...directoryTranslations.en,
    };
    window.translations.ar = {
      ...window.translations.ar,
      ...directoryTranslations.ar,
    };
  }
  initPage("reports");
  syncReportsData();
  ...
  if (
    user &&
    (user.role === "hr" ||
      (window.normalizeRole && window.normalizeRole(user.role) === "hr"))
  ) {
  ...
  if (
    requestedTab &&
    ["sales", "timeline", "coverage", "doctors", "pharmacies"].includes(
      requestedTab,
    )
  ) {
...
document.addEventListener("click", (e) => {
  ...
  if (
    monthContainer &&
    !monthContainer.contains(e.target) &&
    typeof closeMonthDropdown === "function"
  ) {
    closeMonthDropdown();
  }
});
  ...
  if (isAdmin) {
    ...
    if (noticeText)
      noticeText.textContent =
        lang === "ar"
          ? "صلاحية الإدارة: متاح رفع شيتات المبيعات وإدارة الأطباء والصيدليات"
          : "Admin Role: Full Sales Upload, Doctors & Pharmacies Management";
    ...
    if (noticeText)
      noticeText.textContent =
        lang === "ar"
          ...
          : "View-Only: Directories and Sales managed by Admin";
...
function initAllFilters(user) {
  ...
  const role = window.normalizeRole
    ...
    : ((user && user.role) || "medical_rep").toLowerCase();
  const userLines =
    typeof window.getUserLines === "function" && user
      ? window.getUserLines(user.id)
      : [];
  ...
  if (role === "medical_rep") {
    ...
    populateRepsFilter(
      repSelect,
      null,
      hasMultipleLines ? "all" : user.lineId || "line1",
    );
  ...
  const populateSelect = (selectEl) => {
    ...
    if (role === "line_manager") {
      const dms = allUsers.filter(
        (u) => u.managerId === user.id && u.role === "district_manager",
      );
      ...
      optSelf.textContent =
        lang === "ar"
          ? `${user.name} (LM - زياراتي)`
          : `${user.name} (LM - My Visits)`;
      ...
      if (dms.length > 0) {
        ...
        dmGroup.label =
          lang === "ar" ? "مديرو المناطق (DMs)" : "District Managers (DMs)";
        appendSelectOptions(
          dmGroup,
          dms,
          (dm) => dm.id,
          (dm) => `${dm.name} (${dm.employeeCode || "DM"})`,
        );
      ...
      selfOpt.textContent =
        lang === "ar"
          ? `${user.name} (DM - زياراتي)`
          : `${user.name} (DM - My Visits)`;
      ...
      visitRoles.forEach(({ key, label }) => {
        const members = allUsers.filter((u) => {
          const r = window.normalizeRole
            ? window.normalizeRole(u.role)
            : u.role;
          return (
            r === key ||
            u.role === key ||
            (key === "medical_rep" && u.role === "rep")
          );
        });
...
function populateLinesFilter(lineSelect, user = null) {
  ...
  const lang = getCurrentLang();
  const allLinesLabel =
    lang === "ar" ? "جميع خطوط الإنتاج (الكل)" : "All Lines (Select All)";
  ...
  if (user && user.role !== "admin" && user.role !== "business_unit") {
    const userLines =
      typeof window.getUserLines === "function"
        ? window.getUserLines(user.id)
        : [];
...
function populateDMsFilter(dmSelect, lmId = null, lineId = null) {
  ...
  const lang = getCurrentLang();
  const allDMsLabel =
    lang === "ar" ? "جميع المناطق ومديريها (الكل)" : "All Districts & DMs";
  dmSelect.innerHTML = `<option value="all">${allDMsLabel}</option>`;
  const allUsers = (window.DEMO_DATA && window.DEMO_DATA.users) || [];
  let dms = allUsers.filter((u) =>
    window.normalizeRole
      ? window.normalizeRole(u.role) === "district_manager"
  ...
  if (lineId && lineId !== "all") {
    dms = dms.filter(
      (u) => (u.lineIds && u.lineIds.includes(lineId)) || u.lineId === lineId,
    );
  }
  appendSelectOptions(
    dmSelect,
    dms,
    (d) => d.id,
    (d) => `${d.name} (${d.employeeCode || d.code || "DM"})`,
  );
}
...
function populateRepsFilter(repSelect, dmId = null, lineId = null) {
  ...
  const lang = getCurrentLang();
  const allRepsLabel =
    lang === "ar" ? "جميع المناديب (الكل)" : "All Representatives";
  repSelect.innerHTML = `<option value="all">${allRepsLabel}</option>`;
  const allUsers = (window.DEMO_DATA && window.DEMO_DATA.users) || [];
  let reps = allUsers.filter((u) =>
    window.isRepRole
      ? window.isRepRole(u)
  ...
  appendSelectOptions(
    ...
    reps,
    (r) => r.id,
    (r) => `${r.name} (${r.employeeCode || r.code || "Rep"})`,
  );
}
...
// ============================================================================
function populateModalHierarchy(
  lmSelectId,
  dmSelectId,
  repSelectId,
  targetRepId = null,
    ...
    if (rep && rep.managerId) {
      ...
      if (dm && (dm.lineId || dm.managerId)) {
        const line = lines.find(
          (l) => l.lineManagerId === dm.managerId || l.id === dm.lineId,
        );
  ...
  const dms = allUsers.filter(
    (u) =>
      (window.normalizeRole
        ? window.normalizeRole(u.role) === "district_manager"
        : u.role === "district_manager" || u.role === "dm") &&
      (!lmUserId || u.managerId === lmUserId),
  );
  renderSelectOptions(
    dmSelect,
    dms,
    (d) => d.id,
    (d) => `${d.name} (${d.employeeCode || "DM"})`,
  );

  let activeDmId =
    selectedDmId && dms.some((d) => d.id === selectedDmId)
      ? selectedDmId
      : dms[0]?.id || "";
  ...
  const reps = allUsers.filter(
    (u) =>
      (window.isRepRole
        ? window.isRepRole(u)
        : u.role === "medical_rep" || u.role === "rep") &&
      (!currentDmId || u.managerId === currentDmId),
  );
  renderSelectOptions(
    repSelect,
    reps,
